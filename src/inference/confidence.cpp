...
void compute_confidence(whisper_context * ctx, Result & r) {
    ...
    // Tokens con id >= tok_eot son especiales o timestamps en el vocab de whisper.
    ...
    for (size_t i = 0; i < r.segments.size(); ++i) {
        ...
        for (int t = 0; t < n_tokens; ++t) {
            ...
            if (td.id >= tok_eot) continue;  // saltar timestamps y especiales
            sum_p += td.p;
            ++n_valid;
    ...
    // Top-K segmentos peores por min_token_p, solo si están por debajo del umbral.
...
const wchar_t * tier_label(ConfidenceTier t) {
    switch (t) {
        case ConfidenceTier::Excellent: return L"Excelente";
        case ConfidenceTier::Good:      return L"Buena";
        case ConfidenceTier::Low:       return L"Baja";
    }
    return L"-";
}

unsigned long tier_color(ConfidenceTier t) {
    // COLORREF: 0x00BBGGRR
    switch (t) {
...
std::wstring tier_message(ConfidenceTier t, const std::wstring & detected_language) {
    switch (t) {
        case ConfidenceTier::Excellent:
            return L"La transcripción tiene alta confianza token a token. "
                   L"Probablemente es fiel al audio.";
        case ConfidenceTier::Good:
            return L"Confianza aceptable. Si dudas de algún tramo, puedes retranscribir "
                   L"con \"Retranscribir con más calidad\" (beam search) — usa la misma "
                   L"grabación, no necesitas volver a hablar.";
        case ConfidenceTier::Low: {
            std::wstring m =
                L"Confianza baja. Causas posibles: idioma incorrecto detectado";
            if (!detected_language.empty()) {
                m += L" (whisper detectó '" + detected_language + L"')";
            }
            m += L", ruido de fondo, audio recortado o micrófono distorsionado. "
                 L"Sugerencias: fuerza manualmente el idioma desde el desplegable "
                 L"y vuelve a transcribir, o revisa los tramos marcados abajo.";
            return m;
        }
    }
    return L"";
