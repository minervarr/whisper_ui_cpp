...
std::string format_lrc_time(int64_t ms) {
    ...
    int64_t cc = (total % 1000) / 10;  // centisegundos
    char b[16];
...
std::string format_txt(const Result & r) {
    ...
    for (size_t i = 0; i < r.segments.size(); ++i) {
        out += wide_to_utf8(r.segments[i].text);
        out += "\r\n";
    }
    return out;
}
...
std::string format_csv(const Result & r) {
    ...
    for (const auto & seg : r.segments) {
        ...
        out += times;
        out += csv_escape(wide_to_utf8(seg.text));
        out += "\r\n";
    }
    return out;
}
...
std::string format_tsv(const Result & r) {
    ...
    for (const auto & seg : r.segments) {
        ...
        snprintf(times, sizeof(times), "%lld\t%lld\t",
                 (long long) seg.t0_ms, (long long) seg.t1_ms);
        out += times;
        std::string text = wide_to_utf8(seg.text);
        // tabs y newlines en el texto rompen el formato → reemplazar con espacios.
        for (char & c : text) {
            if (c == '\t' || c == '\r' || c == '\n') c = ' ';
        }
        out += text;
...
std::string format_json(const Result & r) {
    ...
    os << "{\r\n";
    os << "  \"detected_language\": \"" << json_escape(wide_to_utf8(r.detected_language)) << "\",\r\n";
    ...
    for (size_t i = 0; i < r.segments.size(); ++i) {
        ...
        os << "      \"t0_ms\": " << s.t0_ms << ",\r\n";
        os << "      \"t1_ms\": " << s.t1_ms << ",\r\n";
        os << "      \"text\": \"" << json_escape(wide_to_utf8(s.text)) << "\",\r\n";
